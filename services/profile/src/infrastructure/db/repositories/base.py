...
class BaseRepository(Generic[ModelType]):
    ...
    async def create(self, **kwargs: Any) -> ModelType:
        ...
        # Делаем flush вместо commit! 
        # flush отправляет SQL-запрос в базу, база генерирует ID (autoincrement),
        # но транзакция еще НЕ закрыта. Если дальше будет ошибка, всё отменится (rollback).
